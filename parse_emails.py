...
# Paths
# Include relevant command line argument or hardcode the below values
...
email_ext = ''
email_files = list(email_path.glob(f'**/*{email_ext}')) # Gets all emails with the corresponding file type
...
def parse_emails():
    ...
    for i, email_file in enumerate(email_files):
        # Prints number of complete emails in a single, always updating, line
        print(f'{i+1} emails completed out of {len(email_files)}', end='\r')
...
def create_parser():
    ...
    parser.add_argument('--xdecode', default=False, nargs=1,
                        help='runs provided xdecode exe path')

    parser.add_argument('--email_loc', default=False, nargs=1,
                        help='runs with provided email directory')

    parser.add_argument('--email_ext', default=False, nargs=1,
                        help='extension of email files')
...
if __name__ == '__main__':
    ...
    if args.xdecode:
        print(f'Using xdecode path: {args.xdecode[0]}')
        xdecode_path = Path(args.xdecode[0])
    elif xdecode_path == '':
        print('No xdeocde_path given. Exiting')
        exit_bool = True
    else:
        print(f'Using default xdecode path: {xdecode_path}')
    if args.email_loc:
        print(f'Using email path: {args.email_loc[0]}')
        email_location = Path(args.email_loc[0])
    else:
        print(f'Using default email path: {email_location}')
    if args.email_ext:
        print(f'Using email file extension: {args.email_ext[0]}')
        email_ext = args.email_ext[0]
    elif email_ext == '':
        print('No email_ext given. Exiting')
        exit_bool = True
    else:
        print(f'Using default email file extension: {email_ext}')
